...
import { LayoutDashboard, Plus, User, LogOut, BookOpen, ChevronRight } from 'lucide-react'
...
function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-surface-card border-t border-surface-border flex items-center">
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex-1 flex flex-col items-center gap-1 py-3 text-[10px] font-medium transition-colors ${
              isActive ? 'text-uj-400' : 'text-gray-500'
            }`
          }
        >
          {({ isActive }) => (
            <>
              <Icon size={20} className={isActive ? 'text-uj-400' : 'text-gray-500'} />
              <span>{label === 'Nova Ocorrência' ? 'Registrar' : label}</span>
            </>
          )}
        </NavLink>
      ))}
...
export function Sidebar() {
  ...
  return (
    <>
      <aside className="hidden md:flex w-60 shrink-0 h-screen sticky top-0 flex-col border-r border-surface-border bg-surface-card">
        <div className="px-5 py-5 border-b border-surface-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-display font-bold text-white text-sm tracking-tight">UniResolve</span>
